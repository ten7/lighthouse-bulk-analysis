...
try {
  ...
  if (!runDir || !domain || !timestamp) {
    console.error(
      'Missing env: REPORT_DIR, REPORT_DOMAIN, and REPORT_TIMESTAMP are required.'
    );
    process.exit(1);
  }
  ...
  const desktopData = fs.existsSync(desktopPath)
    ? loadManifest(desktopPath)
    : [];
  ...
  console.log(
    `| URL (Mobile / Desktop)               | ${dim}Perf${reset}      | ${dim}Access${reset}    | ${dim}BestPrac${reset}  | ${dim}SEO${reset}       |`
  );
  console.log(
    '|--------------------------------------|-----------|-----------|-----------|-----------|'
  );
  ...
  urls.forEach((url) => {
    ...
    let shortUrl = url
      ...
      .replace('www.', '');
    const cleanShortUrl =
      shortUrl.length > 34 ? shortUrl.substring(0, 31) + '...' : shortUrl;

    const urlObj = new URL(url);
    let niceTitle =
      urlObj.pathname === '/' ? 'Home' : urlObj.pathname.replace(/\//g, ' ').trim();
    if (!niceTitle) niceTitle = 'Home';
    niceTitle = niceTitle.replace(/\b\w/g, (l) => l.toUpperCase());

    const printCell = (m, d) =>
      `${color(m)}${String(m).padStart(3)}${reset}/${color(d)}${String(d).padEnd(3)}${reset}`;
    console.log(
      `| ${cleanShortUrl.padEnd(36)} | ${printCell(mP, dP)} | ${printCell(mA, dA)} | ${printCell(mB, dB)} | ${printCell(mS, dS)} |`
    );
    ...
    const mLink = mRun
      ? `<a href="mobile/${path.basename(mRun.htmlPath)}" target="_blank">Mobile</a>`
      : '<span style="color:#ccc">Mobile</span>';
    const dLink = dRun
      ? `<a href="desktop/${path.basename(dRun.htmlPath)}" target="_blank">Desktop</a>`
      : '<span style="color:#ccc">Desktop</span>';
  ...
  const renderRecommendations = (title, type, items) => {
    ...
    items.forEach((op, index) => {
      ...
      console.log(
        `   ${index + 1}. \x1b[36m${op.title}\x1b[0m (~${seconds}s)`
      );
